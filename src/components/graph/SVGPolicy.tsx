...
const SVGPolicy: React.FunctionComponent<Props> = props => {
  ...
  return (
    ...
    <g
      ...
      ref={ref}
    >
      <rect x={props.x} y={props.y} height={props.width / 2} width={props.width} fill={props.colors.policyBackground}

      />
      {props.erasurePolicy &&
      renderClockButton( props.x + props.width * 0.1, props.y + props.width / 2 * 0.3, props.width * 0.1, props.colors.policyIcon,
        props.colors.policyBackground,
        ( e ) => {
        }, (props.fixedLabels.durationOfStorage || props.language.durationOfStorage!) )
      }
      {props.storagePolicy &&
      renderIconDBButton( props.x + props.width * 0.1, props.y + props.width / 2 * 0.6, props.width * 0.1, props.colors.policyIcon,
        props.colors.policyBackground,
        ( e ) => {
      ...
      {props.erasurePolicy && (
        <SVGText canvasWidth={props.canvasWidth}
                 ...
                 color={props.colors.policyLabel}
                 textAnchor={"start"}
                 colors={props.colors}
        />
      )}
