...
const SVGText: React.FunctionComponent<Props> = props => {
  ...
  let sumRowCount = 0;
  if( props.textOverflow === 'ellipsis' && props.showDetailedModel ) {
    sumRowCount = strArray.map( item => item.length )
      .reduce( ( acc, current, index ) => acc + current + (index > 0 ? 1 : 0), 0 );
  }

  return (
    <>
      {sumRowCount <= 3 && !props.textEllipsis && (
        ...
        >
          ...
          {strArray.map( ( labels: string[], index: number ) => {
            ...
            return labels.map( ( label: string, j: number ) => {
              ...
              const newy = props.y
                + (rowsBefore + j + index) * getAbsoluteValue( props.canvasWidth, props.textSize )
                + props.height/2
